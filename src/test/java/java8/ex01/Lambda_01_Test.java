...
public class Lambda_01_Test {
	...
	private List<Person> filter(List<Person> persons, PersonPredicate predicate) {
		...
		ArrayList<Person> PersonTri = new ArrayList<Person>();
		for (Person person : persons) {
			if (predicate.test(person)) {
				PersonTri.add(person);
			}
	...
	public void test_filter_by_firstname() throws Exception {
		...
		// "first_10"
		List<Person> result = filter(personList, p->p.getFirstname()=="first_10");
	...
	public void test_filter_by_password() throws Exception {
		...
		List<Person> result = filter(personList, p->p.getAge()>49 && p.getPassword().equals(DigestUtils.sha512Hex(passwordSha512Hex)));
